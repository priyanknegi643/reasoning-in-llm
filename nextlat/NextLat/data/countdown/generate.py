...
parser.add_argument(
    "--data_dir", type=str, default="data/countdown", help="Directory to store data"
)

# countdown specific
parser.add_argument(
    "--start_range", type=int, default=3, help="Max Range of starting numbers [M, N]"
)
parser.add_argument(
    "--min_range", type=int, default=3, help="Min Range of starting numbers [M, N]"
)
...
parser.add_argument(
    ...
    type=int,
    default=10000,
    help="Maximum value of intermediate numbers (non-inclusive)",
)
...
parser.add_argument(
    "--grow",
    action="store_true",
    help="grow mode on or off, only a new train set is created",
)
...
if __name__ == "__main__":
    ...
    for split, target_nums in zip(splits, target_list):
        ...
        for t in tqdm.tqdm(range(num_samples)):
            ...
            cd = CountDown(
                args.max_target, start_size, max_intermediate=args.max_intermediate
            )
            max_nodes = None
            # if start_size == 2:
            ...
            max_operations = max(max_operations, len(solution))
            sequence = cd.convert_to_sequence(target, nums, solution)
            # no_backtrack_trace = cd.convert_to_path(target, nums, solution)
            if split == "val":
                while sequence in [s for s in data_samples["train"]]:
                    target = random.choice(target_nums)
                    nums, solution = cd.generate(target)
                    sequence = cd.convert_to_sequence(target, nums, solution)
        ...
        # with open(f"{args.data_dir}/{split}{args.offset}_b{args.start_range}_t{args.max_target}_n{args.num_samples}.json", "w") as f:
        #     json.dump(data_samples[split], f, indent=4)
        os.makedirs(args.data_dir, exist_ok=True)
        with open(
            f"{args.data_dir}/{split}{args.offset}_b{args.start_range}_t{args.max_target}_n{args.num_samples}.txt",
            "w",
        ) as f:
