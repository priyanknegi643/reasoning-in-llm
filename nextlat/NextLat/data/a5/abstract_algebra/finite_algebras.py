...
class Magma(SingleElementSetAlgebra):
    ...
    def left_cosets(self, subalgebra):
        ...
        return map(
            ...
            {frozenset([self.op(x, y) for y in subalgebra]) for x in self},
        )
    ...
    def right_cosets(self, subalgebra):
        ...
        return map(
            lambda s: sorted(list(s)),
            {frozenset([self.op(y, x) for y in subalgebra]) for x in self},
        )
...
def generate_symmetric_group(n, name=None, description=None, base=1):
    ...
    mul_tbl = [
        [rev_elem_dict[elem_dict[a] * elem_dict[b]] for b in elem_dict]
        for a in elem_dict
    ]
