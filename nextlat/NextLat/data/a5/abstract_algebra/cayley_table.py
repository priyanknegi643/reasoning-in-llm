...
class CayleyTable:
    ...
    def __init__(self, arr):
        ...
        if nrows == ncols:
            if (np.min(tmp) >= 0) and (np.max(tmp) < nrows):
                ...
                raise Exception(
                    f"All integers must be between 0 and {nrows - 1}, inclusive."
                )
        else:
            raise Exception(
                f"Input arrays must be square; this one is {nrows}x{ncols}."
            )
    ...
    def inverse_lookup_dict(self, identity):
        ...
        return {
            elements[elem_index]: elements[elem_inv_index]
    ...
    def about(self, printout=False):
        ...
        if printout:
            ...
            print("-" * 85)
            print(
                f"{n :>{6}} {ass :>{11}} {co :>{12}} {lid :>{12}} {rid :>{9}} {ident :>{10}} {invs :>{10}}"
            )
            return None
        else:
...
def about_tables(list_of_cayley_tables):
    ...
    for tbl in list_of_cayley_tables:
        ...
        print(
            f"{i :>{6}} {n :>{6}} {ass :>{11}} {co :>{12}} {lid :>{12}} {rid :>{9}} {ident :>{10}} {invs :>{10}}"
        )
