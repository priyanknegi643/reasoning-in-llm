...
def parse_args():
    ...
    parser.add_argument(
        "--model_name", type=str, default="", help="Model name to run on"
    )
    parser.add_argument(
...
def load_manhattan(folder, do_check=True):
    with open(folder + "/node_and_direction_to_neighbor.pkl", "rb") as f:
        node_and_direction_to_neighbor = pickle.load(f)
    with open(folder + "/valid_turns.pkl", "rb") as f:
...
def load_sequences(folder, model_name, nodes):
    with open(folder + f"/samples/{model_name}.txt", "r") as f:
...
def pipeline(dataset, model_name, nsequences, max_degree, max_distance, randomerr):
    ...
    sequences = load_sequences(dataset, model_name, nodes)
...
if __name__ == "__main__":
    dataset, model_name, nsequences, max_degree, max_distance, randomerr = parse_args()
    identifier = "{}_{}_seq{}_deg{}_dist{}_randerr{}".format(
        model_name, dataset, nsequences, max_degree, max_distance, randomerr
    )
    print(identifier)

    outfile = "graphs/" + identifier + ".pkl"
    reconst_graph = pipeline(
        dataset, model_name, nsequences, max_degree, max_distance, randomerr
    )
